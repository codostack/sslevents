...
export default function EventDesign() {
  ...
  // Custom Animation Classes mapping
  const animClass = {
  ...
  return (
    <div className="w-full min-h-screen bg-white font-['DM_Sans'] flex p-10 lg:p-16">
      ...
      <div className="grid grid-cols-[4fr_5fr_3fr] gap-10 items-stretch w-full min-h-[90vh]">
        ...
        <div className="flex flex-col justify-center py-8">
          ...
          <p className="text-slate-500 text-[0.96rem] leading-[1.85] max-w-[420px]">
            We specialize in crafting unforgettable events that combine creativity,
            precision, and flawless execution. From corporate functions to luxury
            weddings and private celebrations, our team transforms ideas into
            extraordinary experiences. We specialize in crafting unforgettable events... We specialize in crafting unforgettable events that combine creativity,
            precision, and flawless execution. From corporate functions to luxury
            weddings and private celebrations, our team transforms ideas into
            extraordinary experiences. We specialize in crafting unforgettable events...
          </p>
        </div>

        {/* Right Column: Interaction Card */}
        <div className="flex flex-col justify-center h-full">
          <div className="rounded-[20px] overflow-hidden border border-slate-200 shadow-[0_8px_40px_rgba(0,0,0,0.10)] bg-white flex flex-col h-full">
            
            {/* Hero Section of Card */}
            <div className="relative flex-1 min-h-[280px] overflow-hidden flex flex-col justify-end p-5 pb-6 transition-all duration-500" style={{ background: pkg.grad }}>
            ...
            <div className="px-5 py-4 flex items-center gap-2">
              <button className="flex-1 py-[11px] rounded-[10px] text-[12px] font-semibold tracking-wider text-white flex items-center justify-center gap-1.5 transition-all duration-500 active:scale-95 shadow-sm" style={{ background: pkg.accent }}>
                Learn More <ArrowRight size={13} />
              </button>
              <button onClick={prev} className="w-[38px] h-[38px] rounded-[10px] border-[1.5px] border-slate-200 bg-white flex items-center justify-center cursor-pointer flex-shrink-0 hover:bg-slate-50 transition-colors">
                <ChevronLeft size={14} className="text-slate-500" />
              </button>
              <button onClick={nextSlide} className="w-[38px] h-[38px] rounded-[10px] border-[1.5px] border-slate-200 bg-white flex items-center justify-center cursor-pointer flex-shrink-0 hover:bg-slate-50 transition-colors">
              ...
              {services.map((s, i) => {
                ...
                return (
                  <div
                    ...
                    className={`flex-1 h-9 rounded-[10px] flex items-center justify-center cursor-pointer border-[1.5px] transition-all duration-300 relative overflow-hidden ${on ? "bg-opacity-10" : "bg-slate-50 border-transparent"}`}
                    style={{ 
                        borderColor: on ? s.accent : 'transparent', 
                        backgroundColor: on ? `${s.accent}14` : undefined 
                    }}
                  >
